...
<div class="max-w-3xl mx-auto mt-3 px-1">
    <h2 class="text-lg font-semibold mb-6 text-left">Editar informações do departamento</h2>
    ...
    <form action="{{ route('departamentos.update', $departamento->id) }}" method="POST" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="border border-gray-300 rounded-md bg-white shadow-sm overflow-hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 p-5">
                <div>
                    ...
                    <input id="nome_dept" name="nome_dept" type="text"
                           class="{{ $input }} @error('nome_dept') border-red-500 @enderror"
                           value="{{ old('nome_dept', $departamento->nome_dept) }}" placeholder="Ex.: Recursos Humanos" required>
                ...
                <div>
                    ...
                    <input id="sigla" name="sigla" type="text"
                           class="{{ $input }} @error('sigla') border-red-500 @enderror"
                           value="{{ old('sigla', $departamento->sigla) }}" placeholder="Ex.: RH" maxlength="10" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 p-5">
                <div>
                    ...
                    <select id="chefe_dpt_id" name="chefe_dpt_id"
                            class="{{ $input }} @error('chefe_dpt_id') border-red-500 @enderror" required>
                        <option value="">-- Selecione o Chefe --</option>
                        @foreach($funcionarios as $funcionario)
                            <option value="{{ $funcionario->id }}" 
                                {{ old('chefe_dpt_id', $departamento->chefe_dpt_id) == $funcionario->id ? 'selected' : '' }}>
            ...
            <div class="p-5">
                ...
                <textarea id="descricao_dept" name="descricao_dept" rows="3"
                          ...
                          placeholder="Breve descrição das funções do departamento" required>{{ old('descricao_dept', $departamento->descricao_dept) }}</textarea>
